# 📘 PSB Student Platform --- API Contract (Backend Spec)

Этот документ описывает **полный REST API**, который ожидает frontend
проекта `PSB-main` (React).\
Все эндпоинты ниже --- обязательны для корректной работы платформы.

## Общие правила

-   Все ответы --- `application/json`.

-   Авторизация: `Authorization: Bearer <accessToken>`.

-   Ошибки:

    ``` json
    { "error": "Message" }
    ```

------------------------------------------------------------------------

# 0. AUTH & TOKENS

## POST /api/auth/login

Аутентификация пользователя.

### Request

``` json
{
  "email": "student@psb.ru",
  "password": "password123"
}
```

### Response

``` json
{
  "accessToken": "jwt-token",
  "refreshToken": "refresh-token",
  "user": {
    "id": "stu_1",
    "name": "Иван Иванов",
    "email": "student@psb.ru",
    "role": "student"
  }
}
```

------------------------------------------------------------------------

## POST /api/auth/signup

Регистрация нового пользователя.

### Request

``` json
{
  "name": "Иван Иванов",
  "email": "student@psb.ru",
  "password": "password123"
}
```

### Response

*такой же как login*.

------------------------------------------------------------------------

## GET /api/auth/me

Вернуть текущего пользователя.

### Response

``` json
{
  "id": "stu_1",
  "name": "Иван Иванов",
  "email": "student@psb.ru",
  "role": "student"
}
```

------------------------------------------------------------------------

## POST /api/auth/refresh

Обновление токена.

### Request

``` json
{
  "refreshToken": "refresh-token"
}
```

### Response

``` json
{
  "accessToken": "new-token",
  "refreshToken": "new-refresh"
}
```

------------------------------------------------------------------------

# 1. PROFILE

## GET /api/profile

Получение профиля студента.

### Response

``` json
{
  "id": "stu_1",
  "firstName": "Иван",
  "lastName": "Иванов",
  "email": "ivan@psb.ru",
  "phone": "+7 (999) 123-45-67",
  "group": "ИКБО-01-25",
  "university": "МИРЭА",
  "enrolledSince": 2024,
  "completedCourses": 3,
  "timezone": "Europe/Moscow"
}
```

------------------------------------------------------------------------

## PUT /api/profile

Обновление профиля.

### Request

``` json
{
  "firstName": "Иван",
  "lastName": "Иванов",
  "email": "ivan@psb.ru",
  "phone": "+79991234567",
  "group": "ИКБО-01-25",
  "university": "МИРЭА",
  "timezone": "Europe/Moscow"
}
```

### Response

*Обновленный профиль*.

------------------------------------------------------------------------

## GET /api/profile/notifications-settings

### Response

``` json
{
  "emailAssignments": true,
  "emailTests": true,
  "emailNews": false,
  "inAppReminders": true
}
```

------------------------------------------------------------------------

## PUT /api/profile/notifications-settings

### Request

``` json
{
  "emailAssignments": true,
  "emailTests": false,
  "emailNews": false,
  "inAppReminders": true
}
```

### Response

``` json
{ "success": true }
```

------------------------------------------------------------------------

## POST /api/profile/change-password

### Request

``` json
{
  "currentPassword": "oldPass",
  "newPassword": "newPass"
}
```

### Response

``` json
{ "success": true }
```

------------------------------------------------------------------------

# 2. COURSES

## GET /api/student/courses

Опционально: `?status=active|completed|all`.

### Response

``` json
[
  {
    "id": "course_1",
    "title": "Основы финтех-разработки",
    "description": "Изучите основы создания финансовых приложений",
    "instructor": "Анна Смирнова",
    "progressPercent": 65,
    "status": "in_progress",
    "nextDeadline": "2024-12-23T20:59:00Z",
    "badges": ["Хакатон ПСБ"]
  }
]
```

------------------------------------------------------------------------

## GET /api/courses/:courseId/overview

### Response

``` json
{
  "id": "course_1",
  "title": "Основы финтех-разработки",
  "description": "Полное описание курса...",
  "instructor": { "id": "teacher_1", "name": "Анна Смирнова" },
  "progressPercent": 65,
  "modules": [
    {
      "id": "mod_1",
      "title": "Введение в финтех",
      "progressPercent": 100,
      "lessons": 5,
      "completedLessons": 5,
      "status": "completed"
    }
  ],
  "upcomingDeadlines": [
    {
      "type": "assignment",
      "id": "ass_3",
      "title": "ДЗ 3.2",
      "deadline": "2024-12-23T20:59:00Z"
    }
  ]
}
```

------------------------------------------------------------------------

# 3. MATERIALS

## GET /api/courses/:courseId/materials

### Response

``` json
{
  "modules": [
    {
      "id": "mod_1",
      "title": "Введение в финтех",
      "items": [
        {
          "id": "mat_1",
          "type": "video",
          "title": "Знакомство с финтехом",
          "durationMinutes": 12,
          "status": "completed"
        }
      ]
    }
  ]
}
```

------------------------------------------------------------------------

## GET /api/materials/:materialId

### Response (example: video)

``` json
{
  "id": "mat_1",
  "courseId": "course_1",
  "moduleId": "mod_1",
  "type": "video",
  "title": "Основы шифрования",
  "description": "Разбор базовых понятий шифрования...",
  "videoUrl": "https://cdn.psb.ru/video.mp4",
  "transcript": "Текстовая расшифровка...",
  "status": "in_progress",
  "viewProgressPercent": 60,
  "relatedAssignments": [
    { "id": "ass_5", "title": "ДЗ: Безопасность" }
  ]
}
```

------------------------------------------------------------------------

## POST /api/materials/:materialId/progress

### Request

``` json
{
  "viewProgressPercent": 100,
  "completed": true
}
```

### Response

``` json
{ "success": true }
```

------------------------------------------------------------------------

# 4. ASSIGNMENTS

## GET /api/courses/:courseId/assignments

### Response

``` json
[
  {
    "id": "ass_1",
    "title": "ДЗ 3.2",
    "module": "Модуль 3",
    "deadline": "2024-12-23T20:59:00Z",
    "status": "submitted",
    "grade": null,
    "hasNewComments": true
  }
]
```

------------------------------------------------------------------------

## GET /api/assignments/:assignmentId

### Response

``` json
{
  "id": "ass_1",
  "courseId": "course_1",
  "title": "ДЗ 3.2",
  "description": "Реализуйте REST API...",
  "module": "Модуль 3",
  "deadline": "2024-12-23T20:59:00Z",
  "status": "submitted",
  "grade": null,
  "maxGrade": 10,
  "teacherFiles": [
    { "id": "file_t1", "name": "spec.pdf", "url": "https://cdn/spec.pdf" }
  ],
  "criteria": [
    { "id": "crit_1", "title": "Корректность API", "max": 4 }
  ]
}
```

------------------------------------------------------------------------

## GET /api/assignments/:assignmentId/submissions

### Response

``` json
[
  {
    "id": "sub_1",
    "createdAt": "2024-12-20T18:30:00Z",
    "status": "graded",
    "grade": 7,
    "commentFromStudent": "Первая версия",
    "files": [
      { "id": "file_1", "name": "hw_v1.zip", "url": "https://cdn/h1.zip" }
    ],
    "teacherComments": [
      {
        "id": "tc_1",
        "author": { "id": "teacher_1", "name": "Анна Смирнова" },
        "createdAt": "2024-12-21T09:10:00Z",
        "text": "Нужно добавить обработку ошибок."
      }
    ]
  }
]
```

------------------------------------------------------------------------

## POST /api/assignments/:assignmentId/submissions

Multipart form-data.

### Fields

-   `comment` --- string\
-   `files[]` --- files

### Response

``` json
{
  "id": "sub_2",
  "createdAt": "2024-12-22T19:00:00Z",
  "status": "submitted",
  "files": [],
  "teacherComments": []
}
```

------------------------------------------------------------------------

# 5. TESTS

## GET /api/courses/:courseId/tests

### Response

``` json
[
  {
    "id": "test_1",
    "title": "Тест по модулю 3",
    "module": "Модуль 3",
    "deadline": "2024-12-25T20:59:00Z",
    "status": "not_started",
    "attemptsUsed": 0,
    "attemptsLimit": 3,
    "bestScorePercent": null
  }
]
```

------------------------------------------------------------------------

## GET /api/tests/:testId

### Response

``` json
{
  "id": "test_1",
  "title": "Тест по модулю 3",
  "timeLimitMinutes": 20,
  "questions": [
    {
      "id": "q1",
      "type": "single_choice",
      "text": "Что такое токен?",
      "options": [
        { "id": "o1", "text": "..." },
        { "id": "o2", "text": "..." }
      ]
    }
  ]
}
```

------------------------------------------------------------------------

## POST /api/tests/:testId/attempts

### Request

``` json
{
  "answers": [
    { "questionId": "q1", "selectedOptionIds": ["o1"] }
  ]
}
```

### Response

``` json
{
  "attemptId": "att_1",
  "scorePercent": 86,
  "passed": true,
  "details": [
    { "questionId": "q1", "correct": true }
  ]
}
```

------------------------------------------------------------------------

# 6. CHAT

## GET /api/courses/:courseId/chat/channels

### Response

``` json
[
  { "id": "ch_general", "name": "Общий чат", "unreadCount": 5 },
  { "id": "ch_homework", "name": "Вопросы по ДЗ", "unreadCount": 1 }
]
```

------------------------------------------------------------------------

## GET /api/chat/channels/:channelId/messages?cursor=

### Response

``` json
{
  "messages": [
    {
      "id": "msg_1",
      "author": { "id": "teacher_1", "name": "Анна", "role": "teacher" },
      "text": "Не забудьте сдать ДЗ",
      "createdAt": "2024-12-20T09:00:00Z"
    }
  ],
  "nextCursor": null
}
```

------------------------------------------------------------------------

## POST /api/chat/channels/:channelId/messages

### Request

``` json
{"text": "Добрый день!"}
```

### Response

``` json
{
  "id": "msg_10",
  "text": "Добрый день!",
  "createdAt": "2024-12-22T10:00:00Z"
}
```

------------------------------------------------------------------------

# 7. GRADES

## GET /api/courses/:courseId/grades

### Response

``` json
{
  "courseId": "course_1",
  "totalScore": 37,
  "maxTotalScore": 50,
  "items": [
    {
      "id": "ass_1",
      "type": "assignment",
      "title": "ДЗ 3.2",
      "module": "Модуль 3",
      "score": 7,
      "maxScore": 10,
      "status": "graded"
    }
  ]
}
```

------------------------------------------------------------------------

# 8. CALENDAR

## GET /api/calendar?from=&to=

### Response

``` json
[
  {
    "id": "ev_1",
    "date": "2024-12-23",
    "title": "ДЗ 3.2: API",
    "type": "assignment",
    "course": "Основы финтех-разработки",
    "entityType": "assignment",
    "entityId": "ass_1"
  }
]
```

------------------------------------------------------------------------

# 9. NOTIFICATIONS

## GET /api/notifications

### Response

``` json
[
  {
    "id": "n1",
    "type": "assignment_comment",
    "title": "Комментарий к ДЗ",
    "body": "Преподаватель оставил комментарий",
    "entityType": "assignment",
    "entityId": "ass_1",
    "courseId": "course_1",
    "createdAt": "2024-12-21T10:01:00Z",
    "read": false
  }
]
```

-------------------------------------------------------------------co-----

## POST /api/notifications/:id/read

### Request

``` json
{ "read": true }
```

### Response

``` json
{ "success": true }
```

------------------------------------------------------------------------

# ✔ Итог

Это **полная спецификация API**, необходимая фронтенду для работы
платформы PSB Student Platform.
